menu(type='*' where=(sel.count or wnd.is_taskbar or wnd.is_edit) title=title.terminal sep='top' image=icon.run_with_powershell)
{
	...
	item(title=title.Windows_Terminal tip=tip_run_admin admin='True' image='C:\Users\Admin\scoop\apps\windows-terminal\current\wt.exe' cmd='wt.exe' arg='-d "@sel.path\."')
	...
	item(title=title.windows_powershell admin=has_admin tip=tip_run_admin image cmd='C:\Users\Admin\scoop\apps\pwsh\current\pwsh.exe' args='-noexit -command Set-Location -Path "@sel.dir\."')
